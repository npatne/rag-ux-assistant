...
class Session:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Failed to initialize LLM client (attempt {attempt+1}): {str(e)}. Retrying...")
                    time.sleep(retry_delay)
                    retry_delay *= 2
                else:
                    logger.error(f"Failed to initialize LLM client after {max_retries} attempts")
    ...
    def add_chat(self, user_query, llm_answer, section_context=None):
        ...
        if not user_query or not llm_answer:
            logger.warning(f"Attempted to add empty chat to session {self.session_id}")
            return
        ...
        try:
            ...
            if total_tokens > self.token_limit and not self.is_summarizing:
                threading.Thread(target=self._summarize_chats, daemon=True).start()
                
        except Exception as e:
            logger.error(f"Error adding chat to session {self.session_id}: {str(e)}")
    ...
    def _summarize_chats(self):
        ...
        if not self.summary_lock.acquire(blocking=False):
            logger.info(f"Summarization already in progress for session {self.session_id}")
            return
            
        try:
            self.is_summarizing = True
            logger.info(f"Starting summarization for session {self.session_id}")
            
            # Only proceed if LLM client is available
            if not self.llm_client:
                logger.warning(f"Cannot summarize session {self.session_id}: LLM client unavailable")
                return
            ...
            if not chat_text:
                logger.warning(f"No valid chat text to summarize for session {self.session_id}")
                return
            ...
            try:
                ...
                if new_summary:
                    ...
                    logger.info(f"Successfully summarized session {self.session_id}, kept last 5 chats")
                else:
                    logger.warning(f"Empty summary returned for session {self.session_id}")
            except Exception as e:
                logger.error(f"LLM summarization failed for session {self.session_id}: {str(e)}")
                
        except Exception as e:
            logger.error(f"Error during chat summarization for session {self.session_id}: {str(e)}")
            traceback.print_exc()
        finally:
            self.is_summarizing = False
    ...
    def get_context(self) -> str:
        ...
        try:
            ...
            logger.error(f"Error getting context for session {self.session_id}: {str(e)}")
            return ""
...
class SessionManager:
    def __init__(self):
        ...
        if not self.initialization_event.wait(timeout=30):
            logger.warning("UXChatAssistant initialization is taking longer than expected")
        ...
        for attempt in range(max_retries):
            try:
                ...
                for _ in range(self.max_workers):
                    worker = threading.Thread(target=self._process_request_worker, daemon=True)
                    worker.start()
                ...
                if attempt < max_retries - 1:
                    logger.warning(f"Failed to initialize UXChatAssistant (attempt {attempt+1}): {str(e)}. Retrying in {retry_delay}s...")
                    time.sleep(retry_delay)
                    retry_delay *= 2
                else:
                    logger.error(f"Failed to initialize UXChatAssistant after {max_retries} attempts: {str(e)}")
    ...
    def get_or_create_session(self, session_id, mode='general', active_case_study=None):
        ...
        with self.sessions_lock:
            if session_id not in self.sessions:
                logger.info(f"Creating new session: {session_id}, mode: {mode}, case study: {active_case_study}")
    ...
    def handle_query(self, session_id, user_query, active_case_study=None, section_context=None, mode='general'):
        ...
        try:
            if not self.initialization_event.is_set():
                logger.warning("Session manager not fully initialized yet, waiting...")
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    logger.error(f"Error in worker processing request: {str(e)}")
                    if callback:
                        callback(self._create_fallback_response(f"Processing error: {str(e)}"))
                ...
                time.sleep(1)  # Prevent CPU spike if there's a persistent error
        ...
        while self.running:
            try:
                ...
                with self.sessions_lock:
                    ...
                    if to_remove:
                        logger.info(f"Cleaning up {len(to_remove)} inactive sessions")
